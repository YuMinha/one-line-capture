...
- **파싱은 사용자 타임존(KST) 기준, 저장은 UTC.** 이 둘을 섞으면 안 된다.
  `Clock`을 `systemUTC()`로 두면 KST 새벽 0~9시에 파서가 "지금"을 아직 전날로 보고,
  `내일 3시`가 하루 덜 밀린 데다 15:00 UTC(= KST 자정)로 계산된다. 실제로 KST 01:13에
  `내일 3시 과제 제출`을 넣으면 마감이 **15시간 어긋났다.**
  → `Clock`은 `Clock.system(Asia/Seoul)`을 쓰고, 룰이 낸 벽시계 시각을
  `CaptureParser`가 한 곳에서 UTC로 변환해 내보낸다. 지출의 `spent_at`은 날짜 컬럼이라
  변환하지 않는다 — "오늘 쓴 돈"의 오늘은 KST 기준이 맞다.
...
- **타입 필터가 있을 때** — 상세 쪽에서 조회하며 `capture`를 fetch join. **쿼리 1번.**
  (`Capture`에는 상세로 가는 연관이 없다. `@MapsId`로 관계를 상세 쪽이 소유하기 때문에
  `Capture`에서 출발하는 fetch join은 불가능하고, 방향을 뒤집어야 한다.)
- **타입이 섞인 전체 조회** — 한 번에 fetch join이 안 된다. 대신 capture 한 페이지를 가져온 뒤
  그 id들로 타입별로 한 번씩만 더 조회한다. **몇 건이든 쿼리는 최대 4번으로 고정**이라 N+1이 아니다.

> 처음에는 "전체 조회는 상세 없이 요약만"으로 단순화했다가 3주차에 화면을 만들어보고 바꿨다.
> 지출 탭에서는 `9,000원 / 점심`이 보이는데 전체 탭에서는 원문만 보여 정보량이 눈에 띄게 빈약했다.
> 쿼리가 4번으로 고정된다면 치를 값이 아니다. **쿼리 수는 `ListQueryCountTest`가 Hibernate
> 통계로 고정해 둔다** — 건수를 늘려도 쿼리 수가 그대로인지 검사하므로, 나중에 누가 무심코
> 스트림 안에서 상세를 조회하면 즉시 빨개진다.
...
**ExpenseRule** — 금액 표현을 찾으면 지출로 판정하고 `merchant`는 그 표현을 제거한 나머지.

패턴을 표기 형식마다 늘리면 조합이 폭발한다. 대신 **수사를 왼쪽부터 읽는 파서**(`KoreanAmount`)를
하나 두고, 정규식은 "금액이 있을 만한 덩어리"를 집는 일만 한다.

| 입력 | 결과 | 비고 |
|---|---|---|
| `9000원`, `9,000원`, `9000 원` | 9000 | |
| `1만원`, `5천원`, `12만원` | 10000 / 5000 / 120000 | |
| `1.5만원` | 15000 | 소수 + 단위 |
| `1만 5천원`, `1만5천원` | 15000 | 복합 단위 |
| `만원`, `천원`, `백원` | 10000 / 1000 / 100 | 숫자 없는 단위는 1로 읽는다 |
| `오천원`, `만 오천원`, `사천오백원` | 5000 / 15000 / 4500 | 한글 수사 |
| `삼십만원`, `이만 삼천원` | 300000 / 23000 | |
| `₩9,000`, `₩ 9000` | 9000 | 통화 기호만 있고 `원`이 없어도 |
| `점심 9000`, `점심 9,000` | 9000 | **`원` 없는 맨 숫자** — 아래 조건에서만 |

**맨 숫자를 금액으로 볼 조건** (근거가 약하므로 좁게 잡는다):

- 세 자리 이상이거나 콤마가 있을 것 (`방 12 청소`의 12는 제외)
- 뒤에 `시·분·초·월·일·년·주·개·명·번·호·층·칸·%`가 붙지 않을 것 (`3층`, `500개`, `20명`)
- 앞뒤가 `- / : . ,`가 아닐 것 (`9/2`, `2026-08-30`)
- **문장에 날짜·시각 표현이 없을 것** — `오늘 100 계단 오르기`를 100원으로 읽으면 안 된다

> `원`이나 `₩`가 붙어 있으면 이 조건들을 보지 않는다. 단위가 곧 근거다.
...
**오전/오후 표기가 없는 1~11시는 오후로 읽는다** — spec.md §3의 `내일 3시 과제 제출` → `내일 15:00`이 그 근거다.
아침을 뜻하려면 `오전 9시`처럼 써야 한다. 이 규칙은 `9시 회의`를 21:00으로 읽는 대가를 치른다.
...
- ~~**목록 전체 조회 시 상세를 내려줄지.**~~ → **3주차에 판단 완료: 내려준다.** 근거는 §2.5.
